...
public partial class TarifsForm : Form, IDisposable
{
    ...
    private async void UpdateGridAsync()
    {
        ...
        var data = await _context.Tarifs.AsNoTracking().OrderBy(x => x.Id).ToListAsync();
        dataGrid.DataSource = data;
    }

    private async void OnSaveAsync(object sender, EventArgs e)
    {
        var model = _current ?? new Tarif();
        model.PerHour = perHourBox.Value;
        if (_current != null)
        {
    ...
    private async void OnGridDoubleClick(object sender, EventArgs e)
    {
        ...
        _current = await _context.FindAsync<Tarif>(id);
        if (_current == null)
        {
            return;
        }
        perHourBox.Value = _current?.PerHour ?? 0;
